...
async function handleReviewApproved(
  ...
  const { review, facility, providerEmail, providerProfile, seekerEmail, seekerProfile } = data;
  ...
  if (resend && providerEmail) {
    try {
      await resend.emails.send({
        from: "RehabLookup <no-reply@rehablookup.com>",
        to: [providerEmail],
        subject: `New ${review.rating}-Star Review for ${facility.name}`,
        html: generateProviderReviewEmail(review, facility, seekerProfile, providerProfile),
      });
...
function generateProviderReviewEmail(review: any, facility: any, seekerProfile: any, providerProfile: any): string {
  return `
    <!DOCTYPE html>
    <html>
      <head><meta charset="utf-8"></head>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1a1a1a; margin: 0; padding: 0; background-color: #f6f8fb;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #1B365D 0%, #2a4a7a 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: #ffffff; margin: 0; font-size: 24px;">New Review Published!</h1>
          </div>
          <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px;">
            ...
            <p>You can respond to this review from your provider dashboard.</p>
            <p style="margin-top: 20px;">
              <a href="https://rehablookup.com/provider/reviews" style="background: #1B365D; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">View Review</a>
            </p>
          </div>
        </div>
      </body>
