...
function generateMainSitemap(): string {
  ...
  console.log(`[Sitemap] Generated main sitemap with ${allRoutes.length} URLs`);
...
function generateSitemapIndex(): string {
  ...
  console.log("[Sitemap] Generated sitemap index with 2 child sitemaps");
...
async function generateFacilitiesSitemap(): Promise<string> {
  ...
  if (error) {
    console.error("[Sitemap] Error fetching facilities:", error);
    throw error;
  }
...
`;
  ...
  console.log(`[Sitemap] Generated facilities sitemap with ${facilities?.length || 0} URLs`);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[Sitemap] Request for type: ${type}`);
