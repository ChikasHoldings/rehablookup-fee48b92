...
-- Fix critical bug: leads_provider_view with security_invoker=true + no WHERE clause
-- means providers can only see unlocked leads (RLS blocks locked ones).
-- Restore security_barrier=true with proper WHERE clause so providers see all their leads with masked PII.
