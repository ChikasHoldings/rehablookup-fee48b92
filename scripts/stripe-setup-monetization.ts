 ...
 * ─────────────────────────
 * Idempotent creation of the three Stripe products + prices that
 * back the new flat-fee monetization model:
 *
 *   • Pro Annual                       — $1,009.80/yr  (=$99 × 12 × 0.85)
 *   • Featured Add-on Annual           — $6,108.60/yr  (=$599 × 12 × 0.85)
 *   • Concierge Partner Annual         — $10,200.00/yr (=$1,000 × 12 × 0.85)
 *
 * Lookup keys:
 *   rl_pro_annual_v1
 *   rl_featured_annual_v1
 *   rl_concierge_annual_v1
 *
...
const PRODUCTS: ProductSpec[] = [
  {
    ...
    description:
      "Verified listing, direct contact display, leads inbox, review responses. Billed annually.",
    unitAmountCents: 100980, // $1,009.80
  ...
  },
  {
    ...
    description:
      "Phone-rotation placements on state, city, search, treatment, insurance, and article pages. Requires Pro. Billed annually.",
    unitAmountCents: 610860, // $6,108.60
  ...
  },
  {
    ...
    description:
      "Prominent surfacing in concierge-presented matches. Capped 3-5 per major city. Requires Pro. Billed annually.",
    unitAmountCents: 1020000, // $10,200.00
...
async function main() {
  ...
  console.log("[stripe-setup] ensuring 3 products + prices…\n");
  ...
  for (const spec of PRODUCTS) {
    try {
      ...
      console.log(
        `           amount:  $${(spec.unitAmountCents / 100).toLocaleString("en-US", { minimumFractionDigits: 2 })}/yr\n`,
      );
