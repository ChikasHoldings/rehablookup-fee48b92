...
export default function MATClinicNearMe() {
  ...
  return (
    <Layout>
      <SEO
        title={`MAT Clinic Near Me ${stateData ? `in ${stateData.state}` : ""} | Find Treatment Centers`}
        description={`Find medication-assisted treatment (MAT) clinics near you${stateData ? ` in ${stateData.state}` : ""}. Compare ${facilities.length > 0 ? facilities.length + "+" : ""} treatment facilities. Free insurance verification.`}
        canonical={stateSlug ? `/mat-clinic-near-me/${stateSlug}` : "/mat-clinic-near-me"}
        keywords={[
          "mat clinic near me",
          ...
          ...(stateData ? [
            `mat clinic ${stateData.state}`,
            `mat-clinic ${stateData.stateAbbr}`,
          ] : []),
        ]}
        ...
        breadcrumbs={[
          ...
          { name: "MAT Clinic Near Me", url: "/mat-clinic-near-me" },
          ...(stateData ? [{ name: stateData.state, url: `/mat-clinic-near-me/${stateData.slug}` }] : []),
        ]}
      />

      <NearMeHero
        title={`MAT Clinic Near Me${stateData ? ` in ${stateData.state}` : ""}`}
        subtitle={`Find verified medication-assisted treatment clinics${stateData ? ` in ${stateData.state}` : " near you"}. Find MAT programs offering Suboxone, methadone, Vivitrol, and naltrexone for opioid and alcohol use disorders.`}
        treatmentType="Medication-Assisted Treatment Clinics"
        location={stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined}
        facilityCount={facilities.length}
      />
        ...
        <div className="container">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-foreground treatment-intro">
              MAT Clinics {stateData ? `in ${stateData.state}` : "Near You"}
            </h2>
          ...
          <div className="container">
            ...
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {statesData.slice(0, 20).map((state) => (
                <Link
                  key={state.slug}
                  to={`/mat-clinic-near-me/${state.slug}`}
