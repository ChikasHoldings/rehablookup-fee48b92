...
import { getChristianRehabNearMeFAQs } from "@/components/seo/NearMeFAQData";

import { LandingFeaturedSection } from "@/components/featured/LandingFeaturedSection";
export default function ChristianRehabNearMe() {
  const { stateSlug } = useParams<{ stateSlug?: string }>();

  const { facilities, stateData, nearbyStates, locationString, isLoading } = useNearMeFacilities({
    stateSlug,
    basePath: "/christian-rehab-near-me",
  });

  const faqs = getChristianRehabNearMeFAQs(stateData ? { state: stateData.state } : undefined);

  const structuredData = [
    ...generateTreatmentNearMeSchema({
      treatmentType: "Christian Rehabilitation",
      treatmentSlug: "christian-rehab",
      location: stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined,
      facilityCount: facilities.length,
      faqs,
    }),
    generateNearMeSchema({
      serviceType: "Christian Rehabilitation",
      location: stateData 
  ...
  return (
    <Layout>
      <SEO
        title={`Christian Rehab Near Me ${stateData ? `in ${stateData.state}` : ""} | Find Treatment Centers`}
        description={`Find Christian rehabilitation centers near you${stateData ? ` in ${stateData.state}` : ""}. Compare ${facilities.length > 0 ? facilities.length + "+" : ""} treatment facilities. Free insurance verification.`}
        canonical={stateSlug ? `/christian-rehab-near-me/${stateSlug}` : "/christian-rehab-near-me"}
        keywords={[
          "christian rehab near me",
          "faith-based addiction treatment",
          "christian recovery programs",
          "biblical counseling rehab",
          ...(stateData ? [
            `christian rehab ${stateData.state}`,
            `christian-rehab ${stateData.stateAbbr}`,
          ] : []),
        ]}
        ...
        breadcrumbs={[
          ...
          { name: "Christian Rehab Near Me", url: "/christian-rehab-near-me" },
          ...(stateData ? [{ name: stateData.state, url: `/christian-rehab-near-me/${stateData.slug}` }] : []),
        ]}
      />

      <NearMeHero
        title={`Christian Rehab Near Me${stateData ? ` in ${stateData.state}` : ""}`}
        subtitle={`Find verified Christian addiction treatment centers${stateData ? ` in ${stateData.state}` : " near you"}. Find faith-based programs combining biblical principles with evidence-based treatment for lasting recovery.`}
        treatmentType="Christian Rehabilitation"
        location={stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined}
        facilityCount={facilities.length}
      />
      ...
      <LandingFeaturedSection
        ...
        title={stateData ? `Featured Christian Rehab Centers in ${stateData.state}` : `Featured Christian Rehab Centers Near You`}
      />
        ...
        <div className="container">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-foreground treatment-intro">
              Christian Rehab Centers {stateData ? `in ${stateData.state}` : "Near You"}
            </h2>
            <p className="mt-2 text-muted-foreground">
              Browse {facilities.length > 0 ? `${facilities.length > 0 ? facilities.length + "+" : ""}` : ""} treatment facilities offering evidence-based care.
            </p>
          </div>
        ...
        <section className="py-12 bg-muted/30 border-t">
          <div className="container">
            <h2 className="text-2xl font-bold text-foreground mb-6">
              Christian Rehab by State
            </h2>
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {statesData.slice(0, 20).map((state) => (
                <Link
                  key={state.slug}
                  to={`/christian-rehab-near-me/${state.slug}`}
