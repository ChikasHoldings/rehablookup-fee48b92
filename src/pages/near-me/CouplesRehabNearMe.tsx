...
import { getCouplesRehabNearMeFAQs } from "@/components/seo/NearMeFAQData";

import { LandingFeaturedSection } from "@/components/featured/LandingFeaturedSection";
export default function CouplesRehabNearMe() {
  const { stateSlug } = useParams<{ stateSlug?: string }>();

  const { facilities, stateData, nearbyStates, locationString, isLoading } = useNearMeFacilities({
    stateSlug,
    basePath: "/couples-rehab-near-me",
  });

  const faqs = getCouplesRehabNearMeFAQs(stateData ? { state: stateData.state } : undefined);

  const structuredData = [
    ...generateTreatmentNearMeSchema({
      treatmentType: "Couples Rehabilitation",
      treatmentSlug: "couples-rehab",
      location: stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined,
      facilityCount: facilities.length,
      faqs,
    }),
    generateNearMeSchema({
      serviceType: "Couples Rehabilitation",
      location: stateData 
  ...
  return (
    <Layout>
      <SEO
        ...
        description={`Find couples rehabilitation programs near you${stateData ? ` in ${stateData.state}` : ""}. Compare ${facilities.length > 0 ? facilities.length + "+" : ""} treatment facilities. Free insurance verification.`}
        ...
        keywords={[
          ...
          "rehab for couples",
          "couples recovery programs",
          ...(stateData ? [
            `couples rehab ${stateData.state}`,
            `couples-rehab ${stateData.stateAbbr}`,
          ] : []),
        ]}
        ...
        breadcrumbs={[
          ...
          { name: "Couples Rehab Near Me", url: "/couples-rehab-near-me" },
          ...(stateData ? [{ name: stateData.state, url: `/couples-rehab-near-me/${stateData.slug}` }] : []),
        ]}
      />

      <NearMeHero
        title={`Couples Rehab Near Me${stateData ? ` in ${stateData.state}` : ""}`}
        subtitle={`Find verified couples addiction treatment programs${stateData ? ` in ${stateData.state}` : " near you"}. Find specialized programs designed for couples recovering together, featuring joint counseling, relationship therapy, and individualized care plans.`}
        treatmentType="Couples Rehabilitation"
        location={stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined}
        facilityCount={facilities.length}
      />
      ...
      <LandingFeaturedSection
        ...
        title={stateData ? `Featured Couples Rehab Centers in ${stateData.state}` : `Featured Couples Rehab Centers Near You`}
      />
        ...
        <section className="py-12 bg-muted/30 border-t">
          <div className="container">
            <h2 className="text-2xl font-bold text-foreground mb-6">
              Couples Rehab by State
            </h2>
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {statesData.slice(0, 20).map((state) => (
                <Link
                  key={state.slug}
                  to={`/couples-rehab-near-me/${state.slug}`}
