...
export default function InpatientRehabNearMe() {
  ...
  const nearbyStates = useMemo(() => {
    ...
    return statesData
      ...
      .map(s => ({
        name: s.name,
        slug: `/inpatient-rehab-near-me/${s.slug}`,
  ...
  return (
    <Layout>
      <SEO
        title={`Inpatient Rehab Near Me ${stateData ? `in ${stateData.name}` : ""} | Residential Treatment`}
        description={`Find inpatient rehab centers near you${stateData ? ` in ${stateData.name}` : ""}. Compare ${facilities.length}+ residential treatment facilities offering 24/7 care, medical detox, and evidence-based therapy.`}
        ...
        keywords={[
          ...
          ...(stateData ? [
            `inpatient rehab ${stateData.name}`,
            `residential treatment ${stateData.abbr}`,
          ] : []),
        ]}
        ...
        breadcrumbs={[
          ...
          ...(stateData ? [{ name: stateData.name, url: `/inpatient-rehab-near-me/${stateData.slug}` }] : []),
        ]}
      />

      <NearMeHero
        title={`Inpatient Rehab Near Me${stateData ? ` in ${stateData.name}` : ""}`}
        subtitle={`Find residential treatment centers${stateData ? ` in ${stateData.name}` : " near you"} offering 24/7 care, structured programs, and comprehensive addiction recovery services.`}
        treatmentType="Inpatient Rehabilitation"
        location={stateData ? { state: stateData.name, stateAbbr: stateData.abbr } : undefined}
        facilityCount={facilities.length}
      />
      ...
      <section className="py-12 bg-background">
        <div className="container">
          <h2 className="text-2xl font-bold text-foreground mb-2 treatment-intro">
            Inpatient Treatment Centers {stateData ? `in ${stateData.name}` : "Near You"}
          </h2>
