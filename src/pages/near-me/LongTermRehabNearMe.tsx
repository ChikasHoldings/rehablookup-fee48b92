...
import { getLongTermRehabNearMeFAQs } from "@/components/seo/NearMeFAQData";

import { LandingFeaturedSection } from "@/components/featured/LandingFeaturedSection";
export default function LongTermRehabNearMe() {
  const { stateSlug } = useParams<{ stateSlug?: string }>();

  const { facilities, stateData, nearbyStates, locationString, isLoading } = useNearMeFacilities({
    stateSlug,
    basePath: "/long-term-rehab-near-me",
  });

  const faqs = getLongTermRehabNearMeFAQs(stateData ? { state: stateData.state } : undefined);

  const structuredData = [
    ...generateTreatmentNearMeSchema({
      treatmentType: "Long-Term Rehabilitation",
      treatmentSlug: "long-term-rehab",
      location: stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined,
      facilityCount: facilities.length,
      faqs,
    }),
    generateNearMeSchema({
      serviceType: "Long-Term Rehabilitation",
      location: stateData 
  ...
  return (
    <Layout>
      <SEO
        ...
        description={`Find long-term rehabilitation programs near you${stateData ? ` in ${stateData.state}` : ""}. Compare ${facilities.length > 0 ? facilities.length + "+" : ""} treatment facilities. Free insurance verification.`}
        ...
        keywords={[
          ...
          ...(stateData ? [
            `long-term rehab ${stateData.state}`,
            `long-term-rehab ${stateData.stateAbbr}`,
          ] : []),
        ]}
        ...
        breadcrumbs={[
          ...
          { name: "Long-Term Rehab Near Me", url: "/long-term-rehab-near-me" },
          ...(stateData ? [{ name: stateData.state, url: `/long-term-rehab-near-me/${stateData.slug}` }] : []),
        ]}
      />

      <NearMeHero
        title={`Long-Term Rehab Near Me${stateData ? ` in ${stateData.state}` : ""}`}
        subtitle={`Find verified long-term addiction treatment programs${stateData ? ` in ${stateData.state}` : " near you"}. Find 60, 90, and 120+ day residential treatment programs offering comprehensive, extended care for lasting recovery.`}
        treatmentType="Long-Term Rehabilitation"
        location={stateData ? { state: stateData.state, stateAbbr: stateData.stateAbbr } : undefined}
        facilityCount={facilities.length}
      />
      ...
      <LandingFeaturedSection
        ...
        title={stateData ? `Featured Long-Term Rehab Centers in ${stateData.state}` : `Featured Long-Term Rehab Centers Near You`}
      />
        ...
        <section className="py-12 bg-muted/30 border-t">
          <div className="container">
            <h2 className="text-2xl font-bold text-foreground mb-6">
              Long-Term Rehab by State
            </h2>
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {statesData.slice(0, 20).map((state) => (
                <Link
                  key={state.slug}
                  to={`/long-term-rehab-near-me/${state.slug}`}
