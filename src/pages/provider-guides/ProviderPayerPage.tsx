...
import { providerPayerConfigs } from "@/data/providerPayerConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderPayerPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerPayerConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Payer Relations", href: "/rehab-marketing" },
        { label: config.label },
      ]}
