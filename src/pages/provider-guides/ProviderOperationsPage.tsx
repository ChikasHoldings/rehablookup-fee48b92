...
import { providerOperationsConfigs } from "@/data/providerBusinessConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderOperationsPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerOperationsConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      relatedLinks={[
        ...
        { href: "/provider-guides/reduce-empty-beds-rehab", label: "Reduce Empty Beds" },
        ...providerOperationsConfigs
