...
import { providerAccreditationConfigs } from "@/data/providerAccreditationConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderAccreditationPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerAccreditationConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Accreditation & Quality", href: "/rehab-marketing" },
        { label: config.label },
      ]}
