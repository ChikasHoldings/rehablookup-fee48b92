...
import { providerBillingConfigs } from "@/data/providerBillingConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderBillingPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerBillingConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Insurance & Billing", href: "/rehab-marketing" },
        { label: config.label },
      ]}
