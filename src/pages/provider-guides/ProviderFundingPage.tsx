...
import { providerFundingConfigs } from "@/data/providerFundingConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderFundingPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerFundingConfigs.find(c => c.slug === slug);
