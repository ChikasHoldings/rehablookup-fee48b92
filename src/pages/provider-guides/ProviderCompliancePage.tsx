...
import { providerComplianceConfigs } from "@/data/providerComplianceConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderCompliancePage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerComplianceConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Compliance", href: "/rehab-marketing" },
        { label: config.label },
      ]}
