...
import { providerPainPointConfigs } from "@/data/providerPersonaConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderPainPointPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerPainPointConfigs.find(c => c.slug === slug);
