...
import { providerCrisisConfigs } from "@/data/providerCrisisConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderCrisisPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerCrisisConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Crisis & Emergency", href: "/rehab-marketing" },
        { label: config.label },
      ]}
