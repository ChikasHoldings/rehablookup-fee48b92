...
import { providerClinicalConfigs } from "@/data/providerClinicalConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderClinicalPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerClinicalConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Clinical Operations", href: "/rehab-marketing" },
        { label: config.label },
      ]}
