...
import { providerStartupConfigs } from "@/data/providerStartupConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderStartupPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerStartupConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Startup & Expansion", href: "/rehab-marketing" },
        { label: config.label },
      ]}
