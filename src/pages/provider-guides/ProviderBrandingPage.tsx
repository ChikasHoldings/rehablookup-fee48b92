...
import { providerBrandingConfigs } from "@/data/providerBrandingConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderBrandingPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerBrandingConfigs.find(c => c.slug === slug);
