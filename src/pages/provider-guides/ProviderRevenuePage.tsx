...
import { providerRevenueConfigs } from "@/data/providerRevenueConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderRevenuePage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerRevenueConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Revenue", href: "/rehab-marketing" },
        { label: config.label },
      ]}
