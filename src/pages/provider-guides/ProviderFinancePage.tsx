...
import { providerFinanceConfigs } from "@/data/providerDigitalConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderFinancePage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerFinanceConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      relatedLinks={[
        ...
        { href: "/provider-guides/rehab-revenue-diversification", label: "Revenue Diversification" },
        ...providerFinanceConfigs
