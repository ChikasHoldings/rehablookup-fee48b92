...
export default function ProviderHighKeywordPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = highKeywordConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      relatedLinks={[
        { href: "/for-providers", label: "List Your Facility Now" },
        { href: "/rehab-marketing", label: "Rehab Marketing Hub" },
        { href: "/provider-roi-calculator", label: "Calculate Your ROI" },
        { href: "/provider-guides/rehab-local-seo-domination", label: "Local SEO Guide" },
        ...highKeywordConfigs
