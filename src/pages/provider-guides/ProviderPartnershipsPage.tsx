...
import { providerPartnershipsConfigs } from "@/data/providerPartnershipsConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderPartnershipsPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerPartnershipsConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Partnerships", href: "/rehab-marketing" },
        { label: config.label },
      ]}
