...
import { providerStaffingConfigs } from "@/data/providerStaffingConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderStaffingPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerStaffingConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Staffing & Workforce", href: "/rehab-marketing" },
        { label: config.label },
      ]}
