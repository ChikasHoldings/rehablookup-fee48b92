...
import { providerDigitalConfigs } from "@/data/providerDigitalConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderDigitalPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerDigitalConfigs.find(c => c.slug === slug);
