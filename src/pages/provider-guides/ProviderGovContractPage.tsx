...
import { providerGovContractConfigs } from "@/data/providerGovContractConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderGovContractPage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerGovContractConfigs.find(c => c.slug === slug);
