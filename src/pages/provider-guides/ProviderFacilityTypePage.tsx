...
import { providerFacilityTypeConfigs } from "@/data/providerFacilityTypeConfigs";
import NotFound from "@/pages/NotFound";

export default function ProviderFacilityTypePage() {
  const { pathname } = useLocation();
  const slug = pathname.replace("/provider-guides/", "");
  const config = providerFacilityTypeConfigs.find(c => c.slug === slug);
  ...
  return (
    <ProviderConversionPage
      ...
      breadcrumbs={[
        ...
        { label: "Facility Types", href: "/rehab-marketing" },
        { label: config.label },
      ]}
