...
const AdminReVerificationQueue = lazy(() => import("./pages/admin/AdminReVerificationQueue"));
...
function AppGlobals() {
  useTelClickTracking();
  // RESTORED 2026-05-21: this hook was rolled back during incident triage
  // because we suspected its onAuthStateChange cleanup. The actual root
  // cause turned out to be `useFavorites`' fixed channel name collision
  // (different hook), so this is safe to keep on. Tags the GA4 session
  // with `traffic_type` (internal/external) once the user's role is
  // known. See src/hooks/useGAInternalTrafficFlag.ts.
        ...
        <Suspense fallback={<RouteLoadingFallback />}>
          ...
          <Routes>
            ...
            <Route path="/gabapentin-addiction-treatment" element={<PublicRouteGuard><SubstanceTreatmentPage /></PublicRouteGuard>} />
             ...
             <Route path="/schizophrenia-and-addiction-treatment/:stateSlug/:citySlug" element={<PublicRouteGuard><CoOccurringCityPage /></PublicRouteGuard>} />
             <Route path="/chronic-pain-and-addiction-treatment/:stateSlug/:citySlug" element={<PublicRouteGuard><CoOccurringCityPage /></PublicRouteGuard>} />
