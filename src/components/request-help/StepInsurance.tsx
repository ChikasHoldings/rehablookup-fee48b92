        ...
        >
          {insuranceOptions.map((option) => (
            ...
            >
              ...
              <span className={formData.insuranceType === option.value ? "text-primary font-medium" : ""}>
                {option.label}
              </span>
            </label>
          ))}
      ...
      <div className="space-y-3">
        ...
        >
          {budgetOptions.map((option) => (
            <label
              key={option.value}
              className={`flex items-center justify-center px-3 py-3 border rounded-lg cursor-pointer transition-colors text-center ${
