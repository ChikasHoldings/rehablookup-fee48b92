        ...
        {treatmentTypes.slice(0, 6).map((item) => (
          <PrefetchLink key={item.href} to={item.href} onClick={onNavigate}
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground/80 leading-tight mt-0.5">{item.desc}</p>
            </div>
          </PrefetchLink>
        ))}
        <PrefetchLink to="/treatment-types" onClick={onNavigate}
        ...
        <div className="grid grid-cols-2 gap-0.5">
          {popularLocations.map((loc) => (
            <PrefetchLink key={loc.href} to={loc.href} onClick={onNavigate}
              className="group flex items-center gap-2 rounded-md px-2.5 py-2.5 text-[15px] text-foreground/80 hover:text-foreground hover:bg-muted/40 active:bg-muted/60 transition-colors">
        ...
        <div className="grid grid-cols-2 gap-0.5">
          {nearMePages.map((page) => (
            <PrefetchLink key={page.href} to={page.href} onClick={onNavigate}
              className="group flex items-center gap-2 rounded-md px-2.5 py-2.5 text-[15px] text-foreground/80 hover:text-foreground hover:bg-muted/40 active:bg-muted/60 transition-colors">
              <Search className="h-3.5 w-3.5 text-accent/50 group-hover:text-accent shrink-0" />
              {page.label}
            </PrefetchLink>
          ))}
        </div>
