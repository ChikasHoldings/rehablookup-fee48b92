...
import { MessageCircle, Send, User, HeadphonesIcon, Loader2, Paperclip } from "lucide-react";
...
interface AdvisorMessagingProps {
  inquiryId: string;
}
...
export function AdvisorMessaging({ inquiryId }: AdvisorMessagingProps) {
  ...
  // Fetch or create advisor thread
  const { data: thread, isLoading: threadLoading } = useQuery({
    queryKey: ["advisor-thread", inquiryId],
    queryFn: async () => {
  ...
  const { data: messages, isLoading: messagesLoading } = useQuery({
    queryKey: ["advisor-messages", thread?.id],
    queryFn: async () => {
      if (!thread?.id) return [];
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        () => {
          queryClient.invalidateQueries({ queryKey: ["advisor-messages", thread.id] });
        }
      )
      .subscribe();
  ...
  return (
    <Card className="flex flex-col h-[350px] sm:h-[420px]">
      <CardHeader className="border-b py-3">
        ...
        <p className="text-sm text-muted-foreground">
          All communication about your placement goes through your dedicated advisor.
        </p>
      </CardHeader>
        ...
        {messagesLoading ? (
          ...
          <div className="text-center text-muted-foreground py-8">
            ...
            <p className="text-sm mt-1">
              Your advisor will help coordinate calls, tours, and next steps with facilities.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
