  ...
  const { data: selectedCase } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("concierge_inquiries")
        .select("id, user_name, user_email, user_phone, status, payment_status, primary_concern, level_of_care, insurance_carrier, budget_range, preferred_state, preferred_city, timeline_urgency, gender, age_range, notes, admin_notes, assigned_advisor_id, matched_facility_ids, admin_matched_facility_ids, placed_facility_id, created_at, updated_at, intake_data, match_scores, relationship_to_seeker, detox_needed, prior_treatment_history, prior_treatment_notes, substance_use_duration, substance_use_frequency, faith_based_preference, preferred_environment, co_occurring_concerns, amenity_preferences, desired_location_state, desired_location_city, desired_radius_miles, willing_to_travel, current_medications, mobility_needs, scholarship_interest, holistic_interest, referral_source, payment_amount_cents, introductions_sent_at, introductions_sent_count, matched_at, closed_at, placement_confirmed, placement_confirmed_at, seeker_confirmed, seeker_confirmed_at, seeker_feedback, seeker_rating, abandoned_cart_email_sent_at, alternative_contact_name, alternative_contact_phone, assessment_preference, benefits_verified, best_time_to_call, checkout_session_id, current_living_situation, decision_maker_name, decision_maker_phone, draft_id, email_verified_at, emergency_contact_name, emergency_contact_phone, employer_name, form_completed_at, hipaa_consent, idempotency_key, insurance_group_number, insurance_member_id, intake_submitted_at, match_count, needs_transport_help, payment_reminder_count, payment_type, preferred_language, provider_fee_cents, provider_fee_status, provider_fee_type, provider_invoice_id, relationship_to_decision_maker, stripe_customer_id, stripe_payment_intent_id, suicide_history, user_id")
