...
function ModalNextActionBar({ caseData, onRefresh, onSwitchTab }: {
  ...
  const needsAdvisor = !caseData.assigned_advisor_id && !isAdvisor;

  return (
    <div className="mt-3 space-y-2">
      {/* Inline Advisor Assignment when unassigned */}
      {needsAdvisor && (
        <InlineAdvisorAssign caseData={caseData} onRefresh={onRefresh} />
      )}
...
function InlineAdvisorAssign({ caseData, onRefresh }: { caseData: ConciergeInquiry; onRefresh: () => void }) {
  ...
  return (
    <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 flex items-center gap-3">
      <AlertTriangle className="h-4 w-4 text-destructive shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-destructive">No Advisor Assigned</p>
      </div>
      <select value={selectedId} onChange={e => setSelectedId(e.target.value)}
        className="h-8 text-xs rounded-md border bg-background px-2 max-w-[180px]">
        <option value="">Select advisor...</option>
        {staff?.map(s => (
          <option key={s.user_id} value={s.user_id}>
            {s.display_name || `${s.first_name} ${s.last_name}`} ({s.admin_role})
          </option>
        ))}
      </select>
      <Button size="sm" className="h-8 text-xs" disabled={!selectedId || saving} onClick={assign}>
