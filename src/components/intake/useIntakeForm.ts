...
import { LeadIntakeData, initialFormData, TOTAL_STEPS } from "./types";
import { analytics } from "@/lib/analytics";

const STORAGE_KEY = "centralized_lead_intake";
...
export function useIntakeForm() {
  ...
  // Form state
  const [formData, setFormData] = useState<LeadIntakeData>(initialFormData);
  ...
  const updateFormData = useCallback((updates: Partial<LeadIntakeData>) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      analytics.formSubmit("centralized_intake", true);
      setIsSubmitted(true);
      
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : "Please try again";
      trackAnalytics("form_submit_error", { error: message });
      toast({
