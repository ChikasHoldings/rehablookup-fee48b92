...
| 🟢 Low cluster | ✅ 10 of 14 fixed | **Fixed:** L1 bulk-dialog failure toasts (×14), L2 SeekerShell render-phase redirect, L3 impersonation grant re-validated against the DB on restore (sessionStorage blob no longer trusted), L5 saved-search `.or()` escaping, L6 track-provider-event body-cap + UUID validation, L7 run-smoke-tests stack-leak, L8 stripe-webhook URL from env, L9 Stripe `data.error` surfacing (×4), L12 timeline null-guard, L13 `.nvmrc`→22. (Edge functions deployed; frontend pending Vercel.) **L4 analyzed, not dropped:** 144 unused indexes / 27 MB, but the bulk is trigram indexes on `staged_*` ETL tables needed by the (infrequent) SAMHSA import — not safe to bulk-drop; only ~kB-scale `leads_*`/`analytics_events` ones are clear wins. **Architectural (deferred):** L10 tsconfig strict rollout, L11 SAMHSA pipeline, L14 SEO SSR. |
| 🔵 SEO check | ✅ Fixed | `check:unique-meta` was red (209 duplicate-meta errors across ~50k generated pages). Root cause: 16 SAMHSA double-import duplicate facility rows + multi-location orgs sharing name+city+state. Fix: deduped the 16 rows (backup `facilities_dedup_backup_20260619`, migration `20260619010000`) + the profile generator now disambiguates multi-location `<title>`/`<description>` by street address. Greens on the next CI regen. |
